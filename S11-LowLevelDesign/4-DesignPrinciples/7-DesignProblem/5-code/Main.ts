...
import { PaymentAdapter, PaytmGateway, RazorPayGateway } from "./PaymentProcessor";
...
const paytm = new PaymentAdapter(new PaytmGateway());
const exitPanel = new ExitPanel(paytm);
...
[floor1, floor2, floor3, floor4].forEach((floor) => {
  floor.addSpot(new ParkingSpot("S1", ParkingSpotSize.SMALL));
  floor.addSpot(new ParkingSpot("S2", ParkingSpotSize.MEDIUM));
  floor.addSpot(new ParkingSpot("S3", ParkingSpotSize.COMPACT));
  floor.addSpot(new ParkingSpot("S4", ParkingSpotSize.MEDIUM));
});
